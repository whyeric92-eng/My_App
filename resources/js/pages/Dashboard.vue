    ...
    >
                <div>
                    <div class="flex gap-x-2" v-for="user in users" :key="user.id">
                        <DashboardUserRow :user="user" @update="router.post('/users/update', { user_id: user.id, name: user.name, email:user.email }, { onError: (errors) => { for (const msg of Object.values(errors)) toast.error(String(msg)); } })"/>
                    </div>
                </div>
